define("@staking/result",(require, exports)=>{
...
var result_css_default = import_components3.Styles.style({
  textAlign: "center",
  $nest: {
    "i-label > *": {
      fontSize: ".875rem",
...
var Result = class extends import_components4.Module {
  get message() {
  ...
  async buildLink() {
    if (this.message.txtHash) {
      const chainId = await getWallet().getChainId();
  ...
  async renderUI() {
    ...
    if (this.message.status === "warning") {
      ...
      const section = new import_components4.VStack();
      ...
      const image = await import_components4.Image.create({
        width: "50px",
      ...
      const contentSection = await import_components4.Panel.create();
      ...
      const button = new import_components4.Button(mainSection, {
        width: "100%",
        caption: "Close"
      });
      ...
      mainSection.appendChild(label);
      const section = await import_components4.VStack.create();
...
Result = __decorateClass([
  (0, import_components4.customElements)("staking-result")
], Result);
