...
const getLockedTokenObject = (info: any, tokenInfo: any, tokenMap?: any) => {
  if (info) {
    if (info.lockTokenType == StakingType.ERC20_Token) {
      if (!tokenMap) {
        tokenMap = getTokenMap();
      }
      return tokenMap[tokenInfo.tokenAddress];
    }
    if (info.lockTokenType == StakingType.LP_Token && tokenInfo.lpToken) {
      return tokenInfo.lpToken.object;
    }
    else if (info.lockTokenType == StakingType.VAULT_Token && tokenInfo.vaultToken) {
...
const getLockedTokenSymbol = (info: any, token: any) => {
  if (info) {
    ...
    if (info.lockTokenType == StakingType.LP_Token) {
      return 'LP';
    }
    if (info.lockTokenType == StakingType.VAULT_Token) {
